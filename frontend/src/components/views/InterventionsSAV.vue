<template>
  <div class="main-content">
    <h2>Interventions SAV</h2>
    <p>Suivi des interventions du service après-vente.</p>

    <!-- Tableau des interventions SAV -->
    <div class="table-container">
      <table>
        ...
        <tbody>
          <tr v-for="intervention in interventions" :key="intervention.numero_jeton">
...
export default {
  ...
  data() {
    return {
      interventions: [], // Liste des interventions SAV
    };
  },
  created() {
    this.fetchInterventions(); // Récupérer les données au chargement du composant
  },
  methods: {
    async fetchInterventions() {
      try {
        const response = await axios.get('/api/interventionssav/'); // Appel à l'API Django
        this.interventions = response.data; // Mettre à jour la liste des interventions
      } catch (error) {
